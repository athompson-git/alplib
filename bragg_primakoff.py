...
class BraggPrimakoff:
    def __init__(self, crys: Crystal, ma=0.0001, nsamples=100, energy_res=0.73, abs_sum_natoms=3):
        # Lattice params
        self.crys = crys
        self.a = crys.a
        self.z = crys.z
        ...
        self.absorption_sum = AbsorptionSum(Material(crys.mat_name), n_atoms_side=abs_sum_natoms,
                                            physical_length=power(self.volume,1/3))
    ...
    # Bragg condition for massive axion
    def Ea(self, theta_z, phi, mList):
        ...
        vecG = HBARC_KEV_ANG * self.vecG(mList)
        vecU = self.vecU(theta_z,phi)
        return np.nan_to_num(sqrt(power((np.dot(vecG,vecG) - self.ma**2)/(2*np.dot(vecU,vecG)), 2) + self.ma**2))
    ...
    def BraggPrimakoffAvgPhi(self, theta_z, E1=2.0, E2=2.5, gagamma=1e-10, days_exposure=1.0,
                                use_borrmann=False, fixed_hkl=None, hkl_max=4):
        ...
        def Rate(phi):
            ...
            for hkl in self.GetReciprocalLattice(hkl_max):
                ...
                rate += np.sum(heaviside(ea-self.ma, 0.0) \
                    ...
                    * self.FA_SF_2(hkl, ea) * self.FW(ea, E1, E2) \
                    * (1 / np.dot(self.vecG(hkl), self.vecG(hkl)))) * atten_factor

            return rate
        
        rates = np.array([Rate(this_phi) for this_phi in self.phi_list])
        print("Done with rate = ", np.sum(rates))
        return prefactor * np.sum(rates)/self.nsamples  # fast MC-based integration

    def AtomicPrimakoffRate(self, E1=2.0, E2=2.5, gagamma=1e-10, days_exposure=1.0):
        ...
        r0 = 184.15 * np.exp(1)**(-1/2) * self.z[0]**(-1/3) / M_E
        def AtomicPrimakoffDifferentialRate(Ee, Ea):
            return self.ntargets * self.SolarFluxMassiveALP(Ea, gagamma) * (HBARC**2) \
                * iprimakoff_sigma(Ea*1e-3, gagamma*1e-3, self.ma*1e-3, self.z[0], r0) \
                    * exp(-0.5*power((Ee - Ea)/(self.fwhm/2.35), 2)) / (sqrt(2*pi)*self.fwhm/2.35)
        
        ea_list = np.linspace(1.0, 20.0, 100)
        deltaE = (ea_list[-1] - ea_list[0])/100
        rates = deltaE*np.array([quad(AtomicPrimakoffDifferentialRate, E1, E2, args=(Ea,))[0] for Ea in ea_list])
        return (S_PER_DAY*days_exposure)*np.sum(rates)
